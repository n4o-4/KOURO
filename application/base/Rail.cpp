...
void Rail::Initialize(const std::vector<Kouro::Vector3>& controlPoints, int samplePerSegment)
{
...
void Rail::CreateArcLengthTable(int samplePerSegment)
{
	...
	Kouro::Vector3 prevPoint = CatmullRomPosition(controlPoints_, 0.0f);

	for (int i = 1; i <= totalSamples; ++i)
	{
		float t = static_cast<float>(i) / totalSamples; // 0?1 正規化
		Kouro::Vector3 currPoint = CatmullRomPosition(controlPoints_, t);

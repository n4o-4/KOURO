...
void LockOn::DetectEnemies(const std::vector<std::unique_ptr<Enemy>>& enemies) {
    ...
    for (size_t i = 0; i < std::min(prioritizedEnemies.size(), maxLockCount_); ++i) {
        Enemy* enemy = prioritizedEnemies[i].enemy;
...
LockOn::LockLevel LockOn::GetLockLevel(Enemy* enemy) const {
    // 指定された敵のロックオン情報を探す
    for (const auto& info : lockedEnemiesInfo_) {
        if (info.enemy == enemy) {
...
// ロックオン情報を取得するメソッドを追加
LockOn::LockedEnemyInfo* LockOn::GetLockedEnemyInfo(Enemy* enemy) {
...
Vector3 LockOn::GetLockedEnemyPosition(Enemy* enemy) const {
    for (const auto& info : lockedEnemiesInfo_) {
        if (info.enemy == enemy) {
            // 簡易ロックオンの場合は初期位置を返す

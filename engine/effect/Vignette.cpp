...
namespace Kouro
{
	void Vignette::Initialize(GpuContext context, SrvManager* srvManager)
	{
		// パイプラインの生成
	...
	void Vignette::Draw(uint32_t renderTargetIndex, uint32_t renderResourceIndex)
	{
		// 描画先のRTVのインデックス
	...
	void Vignette::CreateRootSignature(Pipeline* pipeline)
	{
		HRESULT hr;
	...
	void Vignette::CreatePipeLineState(Pipeline* pipeline)
	{

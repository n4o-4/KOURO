...
void GaussianFilter::Initialize(DirectXCommon* dxCommon, SrvManager* srvManager)
{
	// パイプラインの生成
...
void GaussianFilter::Draw(uint32_t renderTargetIndex, uint32_t renderResourceIndex)
{
	// 描画先のRTVのインデックス
...
void GaussianFilter::CreatePipeline()
{
	// ルートシグネチャの生成
...
void GaussianFilter::CreateRootSignature(Pipeline* pipeline)
{
	HRESULT hr;
...
void GaussianFilter::CreatePipeLineState(Pipeline* pipeline)
{

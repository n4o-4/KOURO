...
namespace Kouro
{
	void MotionBlur::Initialize(GpuContext context, SrvManager* srvManager)
	{
		// パイプラインの生成
	...
	void MotionBlur::CreateRootSignature(Pipeline* pipeline)
	{
		HRESULT hr;
	...
	void MotionBlur::CreatePipeLineState(Pipeline* pipeline)
	{
		...
		Microsoft::WRL::ComPtr<IDxcBlob> pixelShaderBlob = dxCommon_->CompileShader(L"Resources/shaders/MotionBlur.PS.hlsl", L"ps_6_0");
	...
	void MotionBlur::CreateMaterial()
	{
		// bufferResourceの生成
		resource_ = DirectXCommon::GetInstance()->CreateBufferResource(sizeof(MotionBlurShader::Material));

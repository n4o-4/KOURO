...
void PostEffect::CreatePipeLine()
{
	...
	// Random
	newPipeline = std::make_unique<Pipeline>();
	RandomPipeLine(newPipeline.get());
	effect.pipelines_.push_back(std::move(newPipeline));

	// LinearFog
	newPipeline = std::make_unique<Pipeline>();
	LinearFogPipeLine(newPipeline.get());
	effect.pipelines_.push_back(std::move(newPipeline));
}
...
void PostEffect::DissolveRootSignature(Pipeline* pipeline)
{
	HRESULT hr;
...
void PostEffect::DissolvePipeLine(Pipeline* pipeline)
{
	DissolveRootSignature(pipeline);
...
void PostEffect::RandomRootSignature(Pipeline* pipeline)
{
	HRESULT hr;
...
void PostEffect::RandomPipeLine(Pipeline* pipeline)
{
	RandomRootSignature(pipeline);
...
void PostEffect::LinearFogRootSignature(Pipeline* pipeline)
{
	HRESULT hr;
...
void PostEffect::LinearFogPipeLine(Pipeline* pipeline)
{
	LinearFogRootSignature(pipeline);

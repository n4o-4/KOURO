...
namespace Kouro
{
	void ColorSpace::Initialize(DirectXCommon* dxCommon, GpuContext* context)
	{
		// パイプラインの生成
	...
	void ColorSpace::Draw(uint32_t renderTargetIndex, uint32_t renderResourceIndex)
	{
		...
		// Cbufferの設定
		cmdList_->SetGraphicsRootConstantBufferView(1, resource_.Get()->GetGPUVirtualAddress());
	...
	{
		// bufferResourceの生成
		resource_ = utils_->CreateBufferResource(sizeof(ColorSpaceShader::Material));

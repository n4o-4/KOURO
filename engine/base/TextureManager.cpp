...
void TextureManager::LoadMaskTexture(const std::string& filePath)
{
	if (textureDatas.contains(filePath))
	{
		return;
	...
	srvManager_->CreateSRVforTexture2D(textureData.srvIndex, textureData.resource.Get(), /*textureData.metadata.format*/DXGI_FORMAT_R24_UNORM_X8_TYPELESS, textureData.metadata.mipLevels);
}

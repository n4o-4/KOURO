...
namespace Kouro
{
	namespace GpuResourceInternal
	{
		...
		Microsoft::WRL::ComPtr<ID3D12Resource> CreateUploadBufferResource(ID3D12Device* device, size_t sizeInBytes)
		{
			...
			// アップロード用バッファの作成
			Microsoft::WRL::ComPtr<ID3D12Resource> resultResource;
			device->CreateCommittedResource(&uploadHeapProperties, D3D12_HEAP_FLAG_NONE, &bufferDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&resultResource));
